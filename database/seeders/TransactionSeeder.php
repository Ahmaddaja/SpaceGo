...
class TransactionSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        for ($month = 1; $month <= 12; $month++) {
            ...
            for ($i = 0; $i < $transactionCountForMonth; $i++) {
                ...
                if (in_array($status, ['capture', 'settlement'])) {
                    ...
                    // Occasionally create expired rentals
                    ...
                    // Mark rak as occupied for successful transactions
                ...
                $orderId = 'ORDER-' . $transactionDate->format('YmdHis') . '-' . str_pad($i + 1, 3, '0', STR_PAD_LEFT);
            ...
            for ($j = 0; $j < $renewalCount; $j++) {
                ...
                if ($successfulTransactions->count() > 0) {
                    ...
                    $renewalOrderId = 'RENEWAL-' . $renewalDate->format('YmdHis') . '-' . str_pad($j + 1, 2, '0', STR_PAD_LEFT);
            ...
            // Add some clearance transactions (pengosongan)
            $clearanceCount = rand(0, 2); // 0-2 clearance transactions per month
            for ($k = 0; $k < $clearanceCount; $k++) {
                $expiredTransactions = Transaction::whereIn('transaction_status', ['capture', 'settlement'])
                    ->where('sewa_berakhir', '<', Carbon::now()->subDays(30))
                    ->get();
        ...
        // Add some current/ongoing transactions for this year (2025)
        $currentYear = Carbon::now()->year;
        $currentMonth = Carbon::now()->month;

        for ($month = 1; $month <= $currentMonth; $month++) {
            $currentTransactionCount = rand(3, 8);

            for ($i = 0; $i < $currentTransactionCount; $i++) {
                $customer = $customers->random();
                $rak = $raks->random();

                $status = ['capture', 'settlement', 'pending'][rand(0, 2)]; // Mainly successful or pending

                $daysInMonth = Carbon::create($currentYear, $month, 1)->daysInMonth;
                $day = rand(1, min(Carbon::now()->day, $daysInMonth));
                $transactionDate = Carbon::create($currentYear, $month, $day, rand(8, 22), rand(0, 59), rand(0, 59));
                ...
                if (in_array($status, ['capture', 'settlement'])) {
                    ...
                    // 50% chance to be ongoing rental
                    if (rand(1, 2) == 1 && $sewaBerakhir->isFuture()) {
                        $isOngoing = true;
                    }
                ...
                $orderId = 'CUR-' . $transactionDate->format('YmdHis') . '-' . str_pad($i + 1, 3, '0', STR_PAD_LEFT);
