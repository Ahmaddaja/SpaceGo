...
@section('content')
    <div class="max-w-7xl mx-auto px-4 py-8 pt-20"> <!-- Tambahkan pt-20 untuk offset navbar -->
                ...
                <div class="bg-white p-4 rounded-lg shadow border-l-4 border-yellow-500">
                    <div class="flex justify-between items-center">
                        <div>
                            ...
                            <p class="text-2xl font-bold">{{ $pendingTransactions->whereNotIn('status', ['settlement', 'capture', 'expire'])->count() }}</p>
                        </div>
        ...
        <!-- Menunggu Pembayaran -->
        @php
            // PERBAIKAN: Filter transaksi yang TIDAK kadaluarsa
            $unpaidTransactions = $pendingTransactions->whereNotIn('status', ['settlement', 'capture', 'expire'])
                ->filter(function($transaction) {
                    // Cek apakah transaksi sudah kadaluarsa berdasarkan waktu
                    $batasWaktu = $transaction->created_at->addHours(24);
                    $isExpired = now()->gt($batasWaktu);
                    
                    // Jika transaksi sudah kadaluarsa, jangan tampilkan di sini
                    if ($isExpired) {
                        // Update status transaksi menjadi expire di database
                        // (ini bisa dilakukan via background job atau saat load halaman)
                        // $transaction->update(['transaction_status' => 'expire']);
                        return false;
                    }
                        ...
                        <table class="min-w-full">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Batas Waktu
                                    </th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-200">
                                @foreach ($unpaidTransactions as $transaction)
                                    @php
                                        ...
                                        $batasWaktu = $transaction->created_at->addHours(24);
                                        $isExpired = now()->gt($batasWaktu);
                                        $isPaid = in_array($transaction->status, ['settlement', 'capture']);
                                    @endphp
            ...
            <div class="mb-10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach ($overdueTransactions as $transaction)
                        ...
                        <div
                            class="bg-white rounded-lg shadow p-6 border {{ $isCritical ? 'border-red-200' : 'border-orange-200' }}">
                            <div class="flex justify-between items-start mb-4">
                                ...
                                <span
                                    class="px-3 py-1 {{ $isCritical ? 'bg-red-100 text-red-800' : 'bg-orange-100 text-orange-800' }} text-xs font-medium rounded-full">
                                    {{ $overdueText }}
                                </span>
                            </div>
                                ...
                                @if ($isCritical)
                                    <div class="p-3 bg-red-50 border border-red-200 rounded-lg">
                                        <div class="flex items-center text-red-700">
                                            ...
                                            <span class="text-sm font-medium">Segera perpanjang untuk menghindari
                                                sanksi</span>
                                        </div>
                                    </div>
                                @endif
                            ...
                            <div class="space-y-3">
                                ...
                                <!-- Tombol Lepas -->
                                <form action="{{ route('customer.tagihan.process-expired', $transaction->id) }}"
                                    method="POST" id="form-lepas-{{ $transaction->id }}" class="lepas-rak-form" data-transaction-id="{{ $transaction->id }}">
                                    @csrf
                                    <input type="hidden" name="transaction_id" value="{{ $transaction->id }}">
                                    <button type="button" onclick="confirmLepasRak({{ $transaction->id }}, '{{ $transaction->rak->nama_rak ?? 'Rak' }}')"
        ...
        <!-- Empty State -->
        @php
            $hasUnpaid = $unpaidTransactions->count() > 0;
        @endphp
        @if (!$hasUnpaid && $expiredTransactions->count() == 0 && $overdueTransactions->count() == 0)
    ...
    @if ($unpaidTransactions->count() > 0)
        <script src="https://app.sandbox.midtrans.com/snap/snap.js" data-client-key="{{ config('midtrans.client_key') }}" async
            defer></script>
    @endif
    ...
    @push('scripts')
        ...
        `;
            ...
            function updatePaymentStatus(result, transactionId = null) {
                ...
                fetch("{{ route('payment.update-status') }}", {
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            // Redirect langsung ke halaman tagihan dengan auto-refresh
                            window.location.href = "{{ route('customer.tagihan') }}?payment_success=true&transaction_id=" + transactionId;
                        } else {
                            showAlert(
            ...
            function confirmLepasRak(transactionId, rakNama) {
                const formId = 'form-lepas-' + transactionId;

                // Gunakan showAlert() dengan pesan yang lebih spesifik
                const alertId = showAlert(
                    `Apakah Anda yakin ingin melepas rak <strong>"${rakNama}"</strong>? Status akan berubah menjadi kadaluarsa dan rak tidak akan bisa digunakan lagi.`,
                    'warning',
                    false,
                    null
                );
            ...
            function submitLepasRak(transactionId, retryCount = 0) {
                // Ambil form berdasarkan ID
                const form = document.getElementById('form-lepas-' + transactionId);
                if (!form) {
                ...
                // Kirim request menggunakan FormData (lebih sesuai untuk form Laravel)
                ...
                fetch(url, {
                    ...
                    .then(data => {
                        console.log('Lepas rak response:', data);
                        ...
                        if (data && data.success) {
                            ...
                            const retryAlertId = showAlert(
                                'Gagal melepas rak: ' + (data?.message || 'Terjadi kesalahan tidak diketahui'),
                                'error',
                                false,
                                null
                            );
                            ...
                            if (retryButton) {
                                ...
                                retryButton.onclick = function() {
                                    ...
                                    submitLepasRak(transactionId, retryCount + 1);
                                };
                            }
                    ...
                    .catch(error => {
                        ...
                        console.error('Error melepas rak:', error);
                        ...
                        const errorAlertId = showAlert(
                            'Terjadi kesalahan: ' + (error.message || 'Tidak dapat terhubung ke server') + '. Silakan coba lagi.',
                            'error',
                            false,
                            null
                        );
            ...
            function checkAndRefreshAfterPayment() {
            ...
            document.addEventListener('DOMContentLoaded', function() {
                checkAndRefreshAfterPayment();
                
                // Cek status pembayaran otomatis jika ada parameter
                const urlParams = new URLSearchParams(window.location.search);
                const autoCheck = urlParams.get('check_payments');
                
                if (autoCheck === 'true') {
                    fetch("{{ route('customer.tagihan.check-overdue') }}")
                        .then(response => {
                            if (response.ok) {
                        ...
                        .catch(error => console.error('Error checking payments:', error));
                }
            });
            ...
            @if (session('success'))
                showAlert('{{ session('success') }}', 'success', true, "{{ route('customer.tagihan') }}");
            @endif
